...
# выводим пути к папкам (dirpath) и на именования файлов (filenames) и
# послеэтого
...
# !ls  # закомментировано, так как не работает в .py

# In[ ]:


# заглянем внутрь sample_data
# !ls / content / sample_data/  # закомментировано
...
# если разбить строку методом .split() по символам \r (возврат к началу
# строки) и \n (новая строка)
...
# пройдемся по этому списку, не забыв создать индекс с помощью функции
# enumerate()
...
# выведем первые 142 символа (если параметр не указывать, выведется все
# содержимое)
print(f1.read(142))
