...
# добавим элемент в определенное место в списке через желаемый индекс
# этого элемента
...
# можно выделить первый элемент, а остальные поместить в переменную со
# звездочкой
Mon, *_ = week
Mon
...
# если в кавычках ничего не указывать, элементы просто соединятся, но
# можно указать любой другой элемент
...
# с помощью метода .count() мы можем посчитать частоту вхождения элемента
# в список
nums_.count(3)
...
# пустой кортеж можно создать с помощью пустых круглых скобок () или
# функции tuple()
...
# функция zip() соединит первые элементы списков, вторые элементы списков
# и т.д.
zip(names, income)
...
# два множества равны, если содержат одинаковые элементы, при этом порядок
# элементов не важен
{"a", "b", "c"} == {"c", "b", "a"}
...
# найдем тех, кто работает или в cv, или в nlp, но не в обеих областях
# одновременно
