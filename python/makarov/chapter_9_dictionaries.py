...
# Словари, наряду со списками, кортежами и множествами, относятся к коллекциям, то есть типам данных, содержащим несколько элементов.
...
# +
# создадим исходный словарь с количеством студентов на
...
# +
# создадим lambda-функцию, которая посчитает длину
...
# +
# с помощью функции zip() поэлементно соединим оба
# списка и преобразуем в словарь
# dict(zip(words, length))

# +
# то же самое можно сделать с помощью функции zip()
...
# +
# для преобразования футов в метры создадим lambda-функцию
# lambda m: m * 0.3048
...
# +
# с помощью функции zip() соединим ключи исходного
# словаря с элементами списка metres
# dict(zip(height_feet.keys(), np.round(metres, 2)))

# +
# то же самое можно выполнить с помощью dict
...
# +
# преобразуем обратно из float в int, но уже через
# dict comprehension
...
# +
# а затем заменим значение внешнего словаря info
...
# +
# с помощью list comprehension удалим точки,
