...
#
# `pip install -r requirements.txt` -- устанавливает зависимости в окружение из файла
#
# 2. Что делает каждая команда в списке ниже?
#
# `conda env list` -- перечисляет существующие окружения
#
# `conda create -n env_name python=3.5` -- создаёт окружение `env_name` с версией питона 3.5
#
# `conda env update -n env_name -f file.yml` -- обновляет окружение `env_name` из файла `file.yml`
#
...
#
# активировать окружение, пото устанавливать либо поштучно (`pip install package_name` / `conda install package_name`), либо через файл с окружением
#
