"""ДЗ 1. Блок 2. Анализ и обработка данных. Раздел 5. Пример: прогноз текучести
кадров.
...
# **Conclusion for numerical variables:**. Numerical variables require log transformation for better prediction. Yeo-Johnson transformation did not show good results. `satisfaction_level` and `average_montly_hours` may probably be used in the model.
...
# The two highest correlations include `number_project` and `average_montly_hours`, and `number_project` and `last_evaluation`. Yet the correlation is low positive.
